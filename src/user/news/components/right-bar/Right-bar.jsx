...
export const RightBar = ({
  ...
  const currentItems = data
    .filter((item) => {
      const scientificNewsNames = {
        uz: "Ilmiy yangiliklar",
        ru: "Научные новости",
        en: "Scientific news",
      };

      const currentLanguage = i18n.language;
      const categoryName = item?.category?.[`name_${currentLanguage}`];

      if (!categoryName) return false;
      const isScientificNews =
        categoryName === scientificNewsNames[currentLanguage];
      ...
      return isScientificNews && isSearchMatch;
    })
